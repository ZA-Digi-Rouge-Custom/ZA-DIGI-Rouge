...
function initCommonModifierPool() {
  modifierPool[ModifierTier.COMMON] = [
    ...
    new WeightedModifierType(modifierTypes.ROGUE_BALL, () => (hasMaximumBalls(PokeballType.ROGUE_BALL) ? 0 : 6), 6),
...
/**
 * Returns zero weight before {@link unlockWave}, then the given weight (or result of weight function).
 * @param unlockWave - First wave (inclusive) at which the modifier can appear
 * @param weight - Weight to apply once unlocked, or a function computing it
 */
function weightFromWave(
  unlockWave: number,
  weight: number | WeightedModifierTypeWeightFunc,
): WeightedModifierTypeWeightFunc {
  return (party: Pokemon[], rerollCount?: number) => {
    if (globalScene.currentBattle.waveIndex < unlockWave) {
      return 0;
    }
    if (typeof weight === "function") {
      return weight(party, rerollCount ?? 0);
    }
    return weight;
  };
}
