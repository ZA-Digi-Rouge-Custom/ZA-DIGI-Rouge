...
import { getClassicFinalBossCandidates } from "#balance/classic-final-boss-species";
...
export class GameMode implements GameModeConfig {
  ...
  getOverrideSpecies(waveIndex: number): PokemonSpecies | null {
    if ((this.isClassic || this.isChallenge) && this.isWaveFinal(waveIndex)) {
      const candidates = getClassicFinalBossCandidates();
      if (candidates.length > 0) {
        return randSeedItem([...candidates]);
      }
    }
