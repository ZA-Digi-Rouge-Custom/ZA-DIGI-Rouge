import { CUSTOM_SPECIES_ID_MAX, CUSTOM_SPECIES_ID_MIN, MAX_TERAS_PER_ARENA } from "#app/constants";
...
export function getPokemonSpecies(species: SpeciesId | SpeciesId[]): PokemonSpecies {
  ...
  if (
    species >= 2000
    || (species >= CUSTOM_SPECIES_ID_MIN && species <= CUSTOM_SPECIES_ID_MAX)
  ) {
    const match = allSpecies.find(s => s.speciesId === species);
    if (!match) {
      throw new Error(`No PokemonSpecies registered for SpeciesId ${species}`);
    }
    return match;
  }
