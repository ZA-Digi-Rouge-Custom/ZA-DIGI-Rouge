...
/** Boosts Fire-type move power for Mega Solar (weather-aware). */
export class MegaSolarFireBoostAbAttr extends MoveTypePowerBoostAbAttr {
  constructor() {
    super(PokemonType.FIRE, 1.5);
  }

  override apply(params: any): void {
    const weather = globalScene.arena.weather;

    if (weather && !weather.isEffectSuppressed()) {
      if (weather.weatherType === WeatherType.RAIN || weather.weatherType === WeatherType.HEAVY_RAIN) {
        (this as any).multiplier = 3;
      } else if (weather.weatherType === WeatherType.SUNNY || weather.weatherType === WeatherType.HARSH_SUN) {
        (this as any).multiplier = 1;
      } else {
        (this as any).multiplier = 1.5;
      }
    } else {
      (this as any).multiplier = 1.5;
    }
...
/** Applies status on defend when hit by non-status moves (Spicy Spray). */
