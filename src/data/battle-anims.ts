...
type MoveAnimAliasConfig = {
  sourceMove: MoveId;
  redTint?: boolean;
};

const moveAnimAliases = new Map<MoveId, MoveAnimAliasConfig>([
  [MoveId.HAPPY_HOUR, { sourceMove: MoveId.ENCORE }],
  [MoveId.TERA_BLAST, { sourceMove: MoveId.DAZZLING_GLEAM }],
  [MoveId.SNOWSCAPE, { sourceMove: MoveId.POWDER_SNOW }],
  [MoveId.BLOOD_MOON, { sourceMove: MoveId.MOONBLAST, redTint: true }],
  [MoveId.TERA_STARSTORM, { sourceMove: MoveId.DAZZLING_GLEAM }],
  [MoveId.BOOM_BUBBLE, { sourceMove: MoveId.BUBBLE }],
  [MoveId.NOVA_BLAST, { sourceMove: MoveId.FLAMETHROWER }],
  [MoveId.GIGA_DESTROYER, { sourceMove: MoveId.MAGNET_BOMB }],
  [MoveId.TERRA_FORCE, { sourceMove: MoveId.HYPER_BEAM }],
  [MoveId.OMNI_HOWL, { sourceMove: MoveId.BOOMBURST }],
  [MoveId.SEARING_SLASH, { sourceMove: MoveId.BITTER_BLADE }],
  [MoveId.FIN_CUTTER, { sourceMove: MoveId.CUT }],
  [MoveId.DRY_NEEDLES, { sourceMove: MoveId.PIN_MISSILE }],
  [MoveId.CACTUS_SMASH, { sourceMove: MoveId.WOOD_HAMMER }],
  [MoveId.THUNDER_SLASH, { sourceMove: MoveId.VOLT_TACKLE }],
  [MoveId.INFINITY_ARROW, { sourceMove: MoveId.THOUSAND_ARROWS }],
  [MoveId.ICE_SLASH, { sourceMove: MoveId.ICE_SPINNER }],
  [MoveId.ICE_BLAST, { sourceMove: MoveId.ICE_SHARD }],
  [MoveId.BLUE_BLASTER, { sourceMove: MoveId.FROST_BREATH }],
  [MoveId.ICE_WOLF_CLAW, { sourceMove: MoveId.METAL_CLAW }],
  [MoveId.GARURU_KICK, { sourceMove: MoveId.MEGA_KICK }],
  [MoveId.DINO_KICK, { sourceMove: MoveId.HIGH_JUMP_KICK }],
  [MoveId.COMPOST_BOMBER, { sourceMove: MoveId.SLUDGE_BOMB }],
  [MoveId.BUG_BLASTER, { sourceMove: MoveId.POISON_GAS }],
  [MoveId.POISON_BUBBLES, { sourceMove: MoveId.SMOG }],
  [MoveId.VIRUS_SKATER, { sourceMove: MoveId.POISON_TAIL }],
  [MoveId.PAGE_FAULT, { sourceMove: MoveId.OUTRAGE }],
  [MoveId.CABLE_CRUSHER, { sourceMove: MoveId.INFESTATION }],
  [MoveId.DARK_NETWORK, { sourceMove: MoveId.FIRE_LASH }],
  [MoveId.BONE_SWEEP, { sourceMove: MoveId.BONE_CLUB }],
  [MoveId.ANVIL_SMASH, { sourceMove: MoveId.EARTHQUAKE }],
  [MoveId.CUSTOM_HURRICANE, { sourceMove: MoveId.HURRICANE }],
  [MoveId.AIR_SLAM, { sourceMove: MoveId.BODY_SLAM }],
  [MoveId.EVIL_WING, { sourceMove: MoveId.WING_ATTACK }],
  [MoveId.NECRO_MAGIC, { sourceMove: MoveId.DREAM_EATER }],
  [MoveId.HORN_BUSTER, { sourceMove: MoveId.HORN_ATTACK }],
  [MoveId.VULCANS_HAMMER, { sourceMove: MoveId.GIGATON_HAMMER }],
  [MoveId.DARK_SHOT, { sourceMove: MoveId.SHADOW_BALL }],
  [MoveId.DRACO_BLITZ, { sourceMove: MoveId.SUNSTEEL_STRIKE }],
  [MoveId.AMBUSH, { sourceMove: MoveId.SUCKER_PUNCH }],
  [MoveId.YUCKY_TONGUE, { sourceMove: MoveId.LICK }],
  [MoveId.MUGEN_CANNON, { sourceMove: MoveId.FLASH_CANNON }],
  [MoveId.LION_SWORD, { sourceMove: MoveId.BEHEMOTH_BLADE }],
  [MoveId.NAIL_CRUSHER, { sourceMove: MoveId.SHADOW_CLAW }],
  [MoveId.DUO_SCISSOR_CLAW, { sourceMove: MoveId.GUILLOTINE }],
  [MoveId.NUCLEAR_PUNCH, { sourceMove: MoveId.METEOR_MASH }],
  [MoveId.TRUMP_SWORD, { sourceMove: MoveId.SACRED_SWORD }],
  [MoveId.FAENG_RUSH, { sourceMove: MoveId.PSYCHIC_FANGS }],
  [MoveId.EXCALIBUR, { sourceMove: MoveId.SACRED_SWORD }],
  [MoveId.BEAM_SHIELD, { sourceMove: MoveId.SPIKY_SHIELD }],
  [MoveId.HEAVENS_KNUCKLE, { sourceMove: MoveId.SWEET_KISS }],
  [MoveId.OMNI_BLAST, { sourceMove: MoveId.ZAP_CANNON }],
  [MoveId.HYPER_STINK_SHOT, { sourceMove: MoveId.BARB_BARRAGE }],
  [MoveId.DEATH_NAIL, { sourceMove: MoveId.SHADOW_CLAW }],
  [MoveId.SOUL_CHOPPER, { sourceMove: MoveId.DIRE_CLAW }],
  [MoveId.DEVILS_DEED, { sourceMove: MoveId.DRAGON_DANCE }],
  [MoveId.CONCERT_CRUSH, { sourceMove: MoveId.PERISH_SONG }],
  [MoveId.WEB_WRECKER, { sourceMove: MoveId.FLASH_CANNON }],
  [MoveId.TRANSCENDENT_SWORD, { sourceMove: MoveId.BEHEMOTH_BLADE }],
  [MoveId.BULLET_HAMMER, { sourceMove: MoveId.GIGATON_HAMMER }],
]);
...
export function initMoveAnim(move: MoveId): Promise<void> {
  return new Promise(resolve => {
    if (moveAnims.has(move)) {
      ...
      const aliasConfig = moveAnimAliases.get(move);
      if (aliasConfig) {
        initMoveAnim(aliasConfig.sourceMove).then(() => {
          const sourceAnim = moveAnims.get(aliasConfig.sourceMove);
          if (!sourceAnim) {
            useDefaultAnim(move, MoveId.TACKLE);
            resolve();
            return;
          }
          if (Array.isArray(sourceAnim)) {
            moveAnims.set(
              move,
              aliasConfig.redTint
                ? [getRedTintedAnimConfig(sourceAnim[0]), getRedTintedAnimConfig(sourceAnim[1])]
                : sourceAnim,
            );
          } else {
            moveAnims.set(move, aliasConfig.redTint ? getRedTintedAnimConfig(sourceAnim) : sourceAnim);
          }
          resolve();
        });
        return;
...
function cloneAnimForTint(source: AnimConfig): AnimConfig {
  const cloned = new AnimConfig();
  cloned.id = source.id;
  cloned.graphic = source.graphic;
  cloned.position = source.position;
  cloned.hue = source.hue;
  cloned.frames = source.frames.map(frameList => frameList.map(frame => new ImportedAnimFrame(frame)));
  // Keep timed events by reference so sound/background events are preserved exactly.
  cloned.frameTimedEvents = source.frameTimedEvents;
  return cloned;
}

function getRedTintedAnimConfig(source: AnimConfig): AnimConfig {
  const tintedAnim = cloneAnimForTint(source);
  for (const frameList of tintedAnim.frames) {
    for (const frame of frameList) {
      if (frame.target !== AnimFrameTarget.GRAPHIC) {
        continue;
      }
      const tone = frame.tone ?? [0, 0, 0, 0];
      frame.tone = [
        Math.min((tone[0] ?? 0) + 110, 255),
        Math.max((tone[1] ?? 0) - 70, -255),
        Math.max((tone[2] ?? 0) - 70, -255),
        tone[3] ?? 0,
      ];
    }
  }
