...
export class PlantHealAttr extends WeatherHealAttr {
  override apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    let backupWeather = globalScene.arena.weather;
    let isSpoofed = false;

    if (user.hasAbility(AbilityId.MEGA_SOL)) {
      backupWeather = globalScene.arena.weather;
      (globalScene.arena as any).weather = {
        weatherType: WeatherType.SUNNY,
        turnsLeft: 0,
        isEffectSuppressed: () => false,
        isImmutable: () => true,
      };
      isSpoofed = true;
    }
    ...
    if (isSpoofed) {
      globalScene.arena.weather = backupWeather;
    }
  ...
  getLevels(user: Pokemon): number {
    if (!globalScene.arena.weather?.isEffectSuppressed()) {
      ...
      if (
        ...
        || user.hasAbility(AbilityId.MEGA_SOL)
      ) {
        return this.stages + 1;
      }
    } else if (user.hasAbility(AbilityId.MEGA_SOL)) {
      return this.stages + 1;
    }
    return this.stages;
  }
...
export function initMoves() {
  (allMoves as Move[]).push(
    ...
    new AttackMove(MoveId.BOOM_BUBBLE, PokemonType.NORMAL, MoveCategory.SPECIAL, 40, 100, 15, 30, 0, 1)
      .ballBombMove()
      .attr(FlinchAttr)
  ,
    new AttackMove(MoveId.PEPPER_BREATH, PokemonType.FIRE, MoveCategory.SPECIAL, 65, 100, 20, 30, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.BURN),
    new AttackMove(MoveId.NOVA_BLAST, PokemonType.FIRE, MoveCategory.SPECIAL, 75, 100, 15, 15, 0, 1)
      .pulseMove()
      .attr(StatusEffectAttr, StatusEffect.BURN),
    new AttackMove(MoveId.GIGA_DESTROYER, PokemonType.FIRE, MoveCategory.SPECIAL, 55, 90, 10, -1, 0, 4)
      .attr(MultiHitAttr, MultiHitType.TWO),
    new AttackMove(MoveId.TERRA_FORCE, PokemonType.FIRE, MoveCategory.SPECIAL, 115, 100, 5, -1, 0, 1)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.OMNI_HOWL, PokemonType.NORMAL, MoveCategory.SPECIAL, 140, 100, 10, -1, 0, 6)
      .soundBased()
      .target(MoveTarget.ALL_NEAR_OTHERS)
  ,
    new AttackMove(MoveId.SEARING_SLASH, PokemonType.FIRE, MoveCategory.PHYSICAL, 90, 95, 10, 10, 0, 1)
      .slicingMove()
      .attr(HighCritAttr)
      .attr(StatusEffectAttr, StatusEffect.BURN),
    new AttackMove(MoveId.FIN_CUTTER, PokemonType.WATER, MoveCategory.PHYSICAL, 70, 100, 20, -1, 0, 9)
      .makesContact(false)
      .slicingMove()
      .attr(HighCritAttr)
  ,
    new AttackMove(MoveId.DRY_NEEDLES, PokemonType.GRASS, MoveCategory.PHYSICAL, 18, 95, 20, -1, 0, 1)
      .makesContact(false)
      .attr(MultiHitAttr, MultiHitType.TWO_TO_FIVE),
    new AttackMove(MoveId.CACTUS_SMASH, PokemonType.GRASS, MoveCategory.PHYSICAL, 115, 100, 15, 35, 0, 3)
      .recklessMove()
      .attr(StatusEffectAttr, StatusEffect.TOXIC)
      .attr(RecoilAttr, false, 0.33)
  ,
    new AttackMove(MoveId.THUNDER_SLASH, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 90, 100, 10, -1, 0, 5)
      .slicingMove()
      .attr(HighCritAttr)
  ,
    new AttackMove(MoveId.INFINITY_ARROW, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 65, 100, 10, 100, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
  ,
    new AttackMove(MoveId.ICE_SLASH, PokemonType.ICE, MoveCategory.PHYSICAL, 70, 100, 17, -1, 0, 9)
      .slicingMove()
      .attr(HighCritAttr)
  ,
    new AttackMove(MoveId.ICE_BLAST, PokemonType.ICE, MoveCategory.SPECIAL, 70, 100, 20, 10, 0, 6)
      .attr(StatusEffectAttr, StatusEffect.FREEZE)
      .attr(FreezeDryAttr)
  ,
    new AttackMove(MoveId.BLUE_BLASTER, PokemonType.ICE, MoveCategory.SPECIAL, 50, 100, 15, 100, 0, 5)
      .attr(StatStageChangeAttr, [Stat.SPD], -1)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.ICE_WOLF_CLAW, PokemonType.ICE, MoveCategory.SPECIAL, 30, 100, 5, -1, 0, 2)
      .attr(MultiHitAttr)
  ,
    new AttackMove(MoveId.GARURU_KICK, PokemonType.ICE, MoveCategory.PHYSICAL, 70, 100, 20, -1, 0, 8)
      .attr(ForceSwitchOutAttr, true)
  ,
    new AttackMove(MoveId.DINO_KICK, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 70, 100, 20, -1, 0, 3)
      .attr(MovePowerMultiplierAttr, (_user, target, _move) =>
        target.getHeldItems().filter(i => i.isTransferable).length > 0 ? 1.5 : 1,
      )
      .attr(RemoveHeldItemAttr, false)
      .edgeCase()
  ,
    new AttackMove(MoveId.COMPOST_BOMBER, PokemonType.POISON, MoveCategory.SPECIAL, 95, 100, 10, 50, 0, 2)
      .ballBombMove()
      .attr(StatusEffectAttr, StatusEffect.POISON)
  ,
    new AttackMove(MoveId.BUG_BLASTER, PokemonType.POISON, MoveCategory.SPECIAL, 70, 90, 20, 10, 0, 4)
      .pulseMove()
      .attr(StatStageChangeAttr, [Stat.ATK], -1),
    new AttackMove(MoveId.POISON_BUBBLES, PokemonType.POISON, MoveCategory.SPECIAL, 40, 100, 20, 30, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.POISON)
  ,
    new AttackMove(MoveId.VIRUS_SKATER, PokemonType.POISON, MoveCategory.PHYSICAL, 70, 100, 10, 100, 0, 1)
      .attr(StatStageChangeAttr, [Stat.SPD], 1, true)
  ,
    new AttackMove(MoveId.PAGE_FAULT, PokemonType.POISON, MoveCategory.PHYSICAL, 120, 100, 10, -1, 0, 2)
      .attr(FrenzyAttr)
      .attr(MissEffectAttr, frenzyMissFunc)
      .attr(NoEffectAttr, frenzyMissFunc)
      .target(MoveTarget.RANDOM_NEAR_ENEMY)
  ,
    new AttackMove(MoveId.CABLE_CRUSHER, PokemonType.POISON, MoveCategory.PHYSICAL, 70, -1, 20, -1, 0, 1)
      .attr(TrapAttr, BattlerTagType.BIND)
  ,
    new AttackMove(MoveId.DARK_NETWORK, PokemonType.POISON, MoveCategory.SPECIAL, 130, 90, 5, 100, 0, 3)
      .attr(StatStageChangeAttr, [Stat.SPATK], -2, true)
  ,
    new AttackMove(MoveId.BONE_SWEEP, PokemonType.GROUND, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 1)
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.ANVIL_SMASH, PokemonType.GROUND, MoveCategory.PHYSICAL, 100, 100, 10, -1, 0, 1)
      .makesContact(false)
      .attr(HitsTagForDoubleDamageAttr, BattlerTagType.UNDERGROUND)
      .attr(MovePowerMultiplierAttr, (_user, target, _move) =>
        globalScene.arena.terrainType === TerrainType.GRASSY && target.isGrounded() ? 0.5 : 1,
      ...
      .target(MoveTarget.ALL_NEAR_OTHERS)
  ,
    new AttackMove(MoveId.CUSTOM_HURRICANE, PokemonType.FLYING, MoveCategory.SPECIAL, 80, 100, 14, 30, 0, 5)
      .windMove()
      .attr(StatStageChangeAttr, [Stat.ACC], -1)
  ,
    new AttackMove(MoveId.AIR_SLAM, PokemonType.FLYING, MoveCategory.PHYSICAL, 85, 100, 15, 30, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
  ,
    new AttackMove(MoveId.EVIL_WING, PokemonType.FLYING, MoveCategory.PHYSICAL, 85, 100, 15, 30, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS),
    new AttackMove(MoveId.NECRO_MAGIC, PokemonType.PSYCHIC, MoveCategory.SPECIAL, 85, 100, 10, -1, 0, 1)
      .triageMove()
      .attr(HitHealAttr)
  ,
    new AttackMove(MoveId.HORN_BUSTER, PokemonType.BUG, MoveCategory.PHYSICAL, 90, 90, 10, 100, 0, 2)
      .attr(StatStageChangeAttr, [Stat.SPD], 1, true)
  ,
    new AttackMove(MoveId.VULCANS_HAMMER, PokemonType.ROCK, MoveCategory.PHYSICAL, 100, 100, 5, 30, 0, 9)
      .makesContact(false)
      .attr(StatusEffectAttr, StatusEffect.BURN),
    new AttackMove(MoveId.DARK_SHOT, PokemonType.GHOST, MoveCategory.SPECIAL, 80, -1, 15, -1, 0, 2)
      .pulseMove()
      .ballBombMove()
  ,
    new AttackMove(MoveId.DRACO_BLITZ, PokemonType.DRAGON, MoveCategory.PHYSICAL, 120, 100, 15, 10, 0, 4)
      .recklessMove()
      .attr(StatusEffectAttr, StatusEffect.BURN)
      .attr(RecoilAttr, false, 0.33)
      .attr(HealStatusEffectAttr, true, StatusEffect.FREEZE),
    new AttackMove(MoveId.AMBUSH, PokemonType.DARK, MoveCategory.PHYSICAL, 40, 100, 10, 100, 3, 3)
      .attr(FlinchAttr)
      .condition(new FirstMoveCondition(), 3)
  ,
    new AttackMove(MoveId.YUCKY_TONGUE, PokemonType.DARK, MoveCategory.PHYSICAL, 80, 100, 15, 45, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
  ,
    new AttackMove(MoveId.MUGEN_CANNON, PokemonType.STEEL, MoveCategory.SPECIAL, 90, 100, 5, 30, 0, 9)
      .pulseMove()
      .attr(StatStageChangeAttr, [Stat.ACC], -1),
    new AttackMove(MoveId.LION_SWORD, PokemonType.STEEL, MoveCategory.PHYSICAL, 75, -1, 10, -1, 0, 8)
      .makesContact(false)
      .slicingMove(),
    new AttackMove(MoveId.NAIL_CRUSHER, PokemonType.STEEL, MoveCategory.PHYSICAL, 90, 100, 5, 50, 0, 2)
      .attr(StatStageChangeAttr, [Stat.DEF], -1)
  ,
    new AttackMove(MoveId.DUO_SCISSOR_CLAW, PokemonType.STEEL, MoveCategory.PHYSICAL, 105, 90, 10, 50, 0, 1),
    new AttackMove(MoveId.NUCLEAR_PUNCH, PokemonType.STEEL, MoveCategory.PHYSICAL, 105, 90, 10, 20, 0, 3)
      .punchingMove()
      .attr(StatStageChangeAttr, [Stat.ATK], 1, true)
  ,
    new AttackMove(MoveId.TRUMP_SWORD, PokemonType.STEEL, MoveCategory.PHYSICAL, 25, 100, 10, -1, 1, 9)
      .makesContact(false)
      .slicingMove()
      .attr(MultiHitAttr)
  ,
    new AttackMove(MoveId.FAENG_RUSH, PokemonType.FAIRY, MoveCategory.PHYSICAL, 80, 95, 10, 30, 0, 7)
      .bitingMove()
      .attr(FlinchAttr)
  ,
    new AttackMove(MoveId.EXCALIBUR, PokemonType.FAIRY, MoveCategory.SPECIAL, 105, -1, 5, -1, 0, 5)
      .slicingMove()
  ,
    new SelfStatusMove(MoveId.BEAM_SHIELD, PokemonType.FAIRY, -1, 10, -1, 4, 6)
      .attr(ProtectAttr, BattlerTagType.SPIKY_SHIELD)
      .condition(failIfLastCondition, 3)
  ,
    new AttackMove(MoveId.HEAVENS_KNUCKLE, PokemonType.FAIRY, MoveCategory.PHYSICAL, 90, 85, 20, -1, 0, 1)
      .makesContact(false)
      .punchingMove(),
    new AttackMove(MoveId.OMNI_BLAST, PokemonType.ELECTRIC, MoveCategory.SPECIAL, 100, 100, 5, 20, 0, 2)
      .attr(MultiStatusEffectAttr, [StatusEffect.BURN, StatusEffect.PARALYSIS, StatusEffect.FREEZE])
  ,
    new AttackMove(MoveId.HYPER_STINK_SHOT, PokemonType.POISON, MoveCategory.SPECIAL, 95, 90, 5, -1, 0, 1)
      .ballBombMove()
      .attr(RemoveBattlerTagAttr, [BattlerTagType.PROTECTED])
      .attr(
        RemoveArenaTagsAttr,
      ...
      .ignoresProtect(),
    new AttackMove(MoveId.DEATH_NAIL, PokemonType.GHOST, MoveCategory.PHYSICAL, 85, 95, 10, -1, 0, 4)
      .attr(RemoveScreensAttr, (_user, target) => (target.isPlayer() ? ArenaTagSide.PLAYER : ArenaTagSide.ENEMY))
  ,
    new AttackMove(MoveId.SOUL_CHOPPER, PokemonType.GHOST, MoveCategory.PHYSICAL, 60, 100, 15, -3, 0, 8)
      .makesContact(false)
      .slicingMove()
      .attr(CritOnlyAttr),
    new SelfStatusMove(MoveId.DEVILS_DEED, PokemonType.DARK, -1, 20, -1, 0, 3)
      .attr(StatStageChangeAttr, [Stat.ATK, Stat.SPD], 1, true),
    new StatusMove(MoveId.CONCERT_CRUSH, PokemonType.DARK, 100, 30, -1, 0, 6)
      .soundBased()
      .reflectable()
      .attr(StatStageChangeAttr, [Stat.ATK, Stat.SPATK], -1)
  ,
    new AttackMove(MoveId.WEB_WRECKER, PokemonType.DARK, MoveCategory.SPECIAL, 115, 100, 5, -1, 0, 5)
      .target(MoveTarget.ALL_NEAR_OTHERS)
      .attr(HitsTagForDoubleDamageAttr, BattlerTagType.UNDERGROUND)
  ,
    new AttackMove(MoveId.TRANSCENDENT_SWORD, PokemonType.STEEL, MoveCategory.PHYSICAL, 240, 100, 1, -1, 0, 8)
      .slicingMove()
      .attr(StatStageChangeAttr, [Stat.EVA], -1)
      .attr(ClearWeatherAttr, WeatherType.FOG)
      .attr(ClearTerrainAttr)
  ...
  ,
    new AttackMove(MoveId.BULLET_HAMMER, PokemonType.STEEL, MoveCategory.PHYSICAL, 150, 100, 5, -1, 0, 9)
      .makesContact(false)
      .attr(RechargeAttr)
      .attr(ForceSwitchOutAttr, false, SwitchType.FORCE_SWITCH),
    new AttackMove(MoveId.SUPREME_CANNON, PokemonType.ICE, MoveCategory.SPECIAL, 95, 100, 10, 25, 0, 3)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.FLASH_FLOOD, PokemonType.WATER, MoveCategory.SPECIAL, 40, 100, 15, -1, 1, 1),
    new AttackMove(MoveId.CHERRY_BLAST, PokemonType.POISON, MoveCategory.SPECIAL, 85, 100, 10, 25, 0, 9)
      .pulseMove()
      .attr(AddBattlerTagAttr, BattlerTagType.CURSED, false)
  );
}
