...
import { shouldUseZarMegaBaseIcon } from "#balance/zar-mega-base-icons";
...
export abstract class PokemonSpeciesForm {
  ...
  getIconAtlasKey(formIndex?: number, shiny?: boolean, variant?: number): string {
    const formSpriteKey = this.getFormSpriteKey(formIndex);
    const iconFormIndex = shouldUseZarMegaBaseIcon(this.speciesId, formSpriteKey) ? 0 : formIndex;
    const variantDataIndex = this.getVariantDataIndex(iconFormIndex);
    const isVariant =
  ...
  getIconId(female: boolean, formIndex?: number, shiny?: boolean, variant?: number): string {
    ...
    const iconFormIndex = shouldUseZarMegaBaseIcon(this.speciesId, this.getFormSpriteKey(formIndex)) ? 0 : formIndex;
    const variantDataIndex = this.getVariantDataIndex(iconFormIndex);
    const replacement = timedEventManager.getEventPokemonSpriteReplacement(this.speciesId, formIndex);
    ...
    if (shouldUseZarMegaBaseIcon(this.speciesId, formSpriteKey)) {
      formSpriteKey = null;
    }
    if (formSpriteKey) {
      switch (this.speciesId) {
