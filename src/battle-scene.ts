...
import { createClassicFinalBossPhase2FormChange } from "#balance/classic-final-boss-species";
    ...
    if (pokemon.isEnemy() && pokemon.isBoss() && !pokemon.formIndex && pokemon.bossSegmentIndex < 1) {
      ...
      this.ui.showDialogue(
        ...
        () => {
          ...
          const phase2FormIndex = this.currentBattle.finalBossPhase2FormIndex;
          if (
            phase2FormIndex != null
            && phase2FormIndex > 0
            && phase2FormIndex < pokemon.species.forms.length
          ) {
            const formChange = createClassicFinalBossPhase2FormChange(
              pokemon.species,
              pokemon.formIndex,
              phase2FormIndex,
            );
            this.phaseManager.unshiftPhase(
              this.phaseManager.create("QuietFormChangePhase", pokemon, formChange),
            );
          } else {
